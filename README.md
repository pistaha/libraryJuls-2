...
Создан прототип электронной библиотеки с клиентской маршрутизацией, локальным каталогом, серверным REST API и интеграцией с ЛитРес. Проект поддерживает запуск в режиме разработки и через Docker Compose. Все обязательные пункты обеих частей задания реализованы.
